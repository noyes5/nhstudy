...
export default function NhEssayStudyApp() {
  ...
  const handleAddQuestion = () => {
    ...
    const defaultCat = selectedCategory === "전체" ? CATEGORIES[0] : selectedCategory;
              ...
              <div className="mt-6 border-t pt-4">
                ...
                <ul className="list-disc list-inside text-sm text-gray-600 space-y-1">
                  {bookmarked.map((id) => {
                    ...
                    const filteredIndex = filteredQuizData.findIndex((fq) => fq.id === id);

                    // 북마크된 문제가 현재 필터에 없으면 표시하지 않거나 클릭 비활성화
                    if (!q || (selectedCategory !== "전체" && q.category !== selectedCategory)) {
                      return null;
                    }
